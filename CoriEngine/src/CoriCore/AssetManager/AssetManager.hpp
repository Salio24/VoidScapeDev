...
namespace Cori {
	...
	public:
		...
		static std::shared_ptr<Texture2D> GetTexture2D(const Texture2DDescroptor& descriptor) { return Get().GetTexture2DImpl(descriptor); }

		static void PreloadShaders(std::initializer_list<ShaderProgramDescriptor> descriptors) { Get().PreloadShadersImpl(descriptors); }
		static void PreloadTexture2Ds(std::initializer_list<Texture2DDescroptor> descriptors) { Get().PreloadTexture2DsImpl(descriptors); }
	...
	private:
		...
		void PreloadTexture2DsImpl(std::initializer_list<Texture2DDescroptor> descriptors);
